...
void ssd1306_draw_square(ssd1306_t *ssd, uint8_t x, uint8_t y, uint8_t cor)
{
    for (uint8_t i = 0; i < 8; i++) {
        for (uint8_t j = 0; j < 8; j++) {
...
{
    if (input_x > 4095) input_x = 4095;  // Limita o valor máximo
    return (input_x * 100) / 4095 + 10;  // Mapeia input_x (0-4095) para x (10-109)
}

uint8_t map_y_to_display(uint16_t input_y) 
{
    if (input_y > 4095) input_y = 4095;  // Limita o valor máximo
